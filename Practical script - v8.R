...
write.csv(training, "training2.csv")
...
bartlett.test(FBS ~ BMI_status, data=training)

# Or, using an additional library
library(car)
leveneTest(FBS ~ BMI_status, training, center=mean)

# Post-hoc test if equal variance [CORRECTED & UPDATED]
TukeyHSD(aov(training$FBS ~ training$BMI_status))
plot(TukeyHSD(aov(training$FBS ~ training$BMI_status)))

# Post-hoc test if can't assume equal variance
library(rstatix)
training %>%  games_howell_test(FBS ~ BMI_status)
